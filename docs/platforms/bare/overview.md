---
title: Bare Runtime Overview
summary: Introduction to the Bare JavaScript runtime, its architecture, and embedding options for cross-device apps.
tags: [bare, runtime, platform]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Bare runtime capabilities, embedding model, and target scenarios.
> - **Primary APIs:** `bare <script>`, Node-compatible module API, ES module imports.
> - **Protocols/Feeds:** Works with Hyper stack components via compatible Node APIs.
> - **Dependencies:** Bare runtime installation (`npm i -g bare`), POSIX-like environment.
> - **Outputs:** Cross-device JavaScript runtime context for Hyper apps and agents.
> - **Next Hop:** [`api.md`](api.md)
...
Bare makes it easy to craft applications that can run effectively across a broad spectrum of devices. To get started, find the Bare API specs [here](https://docs.pears.com/bare-reference/api).

### Modules

Bare provides no standard library beyond the core JavaScript API available through the `Bare` namespace. Instead, there is a comprehensive collection of external modules built specifically for Bare, see [Bare Modules](https://docs.pears.com/bare-reference/bare-modules)
...
## Next Steps
- Explore the module ecosystem in [Bare Modules](bare-modules.md).
- Check platform-specific compatibility notes in [Node.js Compatibility with Bare](node-compatibility.md).
- Review Pear integration guidance in `../pear/pear-runtime.md` when building shared tooling.
