---
title: Node.js Compatibility with Bare
summary: Map of Node.js core modules to Bare equivalents plus notes on remaining gaps.
tags: [bare, nodejs, compatibility]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Node-to-Bare compatibility strategies and remaining gaps.
> - **Primary APIs:** `npm:bare-node-*` aliases, import maps, `bare-*` shim packages.
> - **Protocols/Feeds:** Keeps Hyper services portable between Bare and Node runtime transports.
> - **Dependencies:** Bare runtime, shim packages, optional bundler config.
> - **Outputs:** Compatibility checklist, module replacement table, troubleshooting notes.
> - **Next Hop:** [`bare-modules.md`](bare-modules.md)
...
* `zlib`: [`bare-zlib`](https://github.com/holepunchto/bare-zlib) (through `npm:bare-node-zlib)`
...
- **`dns`**: Use community modules such as [`bare-dns`](https://github.com/holepunchto/bare-dns) or proxy lookups through a service.
- **`cluster`**: Not currently supported; prefer worker processes orchestrated by Autobase or Hyperswarm.
- **`perf_hooks`**: Implement custom timers or integrate with Bare's inspector APIs.

## Next Steps
- Explore module details in [Bare Modules Compatibility Matrix](bare-modules.md).
- Review runtime design via [Bare Runtime Overview](overview.md).
- Document project-specific shims in `../pear/pear-runtime.md` when pairing with Pear apps.

## Consuming dependencies that use core Node.js modules

If the project dependencies use core [Node.js](https://nodejs.org/en/download) modules, use [NPM aliases](#consuming-dependencies-using-npm-aliases) or [import maps](#consuming-dependencies-using-import-maps) to consume the Bare version of those modules.
...
Replace contents of `index.js` with the code from the NPM aliases [example](#example-of-using-fs-in-a-project-using-aliases).
