...
summary: Stage, run, and seed Pear apps so peers can replicate them across the DHT.
tags: [pear, staging, platform]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Stage and seed Pear applications so peers can replicate via the DHT.
> - **Primary APIs:** `pear stage`, `pear run`, `pear seed`, Hyperswarm topics.
> - **Protocols/Feeds:** Application Hypercores, Hyperswarm discovery, Autobase-linked data feeds.
> - **Dependencies:** Pear CLI, Hyperswarm connectivity, staged build outputs.
> - **Outputs:** Staged application storage, running seeders, distribution links.
> - **Next Hop:** [`marking-release.md`](marking-release.md)

Applications can be shared with peers by seeding them to the network from an efficient local data structure (a [Hypercore](../reference/hypercore-api.md)). We call the mirroring of a local file system into the Pear platform Application Storage "staging". Seeding is sharing an app from a machine over the Distributed Hash Table (DHT) (via [Hyperswarm](../reference/hyperswarm-api.md)) so that other peers can replicate, consume and reseed the application.

{% embed url="<https://www.youtube.com/watch?v=BEadqmp7lA0>" %}

Build with Pear - Episode 02: Sharing Pear Applications

This guide can either follow on from [Making a Pear Desktop Application](making-pear-application.md), [Making a Pear Terminal Application](making-pear-terminal-app.md), or get setup quickly with the following:
...
If starting from [Making a Pear Desktop Application](making-pear-application.md) or [Making a Pear Terminal Application](making-pear-terminal-app.md) ensure that the command-line current working directory is set to the project folder of the application.
...
![Trust dialog](https://1301247912-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fjdab83NqbLtX0WX9qY2n%2Fuploads%2Fgit-blob-bd9c63e0185a6a17f249560674b37f07685e7624%2Ftrust-dialog.png?alt=media)
...
## Next Steps
- Promote builds with [Marking a Pear Release](marking-release.md).
- Capture troubleshooting notes in [Debugging Pear Applications](debugging-pear-app.md).
- Keep configuration aligned using [Pear Application Configuration](app-config-and-package-json.md).
