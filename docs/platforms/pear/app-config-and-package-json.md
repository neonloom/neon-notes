---
title: Pear Application Configuration
summary: Details the package.json and pear config fields required to package and run Pear apps.
tags: [pear, configuration, platform]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Define Pear app metadata in `package.json` and `pear` config blocks.
> - **Primary APIs:** `package.json` fields (`name`, `pear`, `main`, `targets`), runtime config options.
> - **Protocols/Feeds:** Governs how apps stage assets, seed over Hyper, and expose capabilities.
> - **Dependencies:** Pear CLI/runtime, Node.js tooling, build scripts.
> - **Outputs:** Valid application manifest, runtime options, packaging directives.
...
## Next Steps
- Review staging behaviour in [Sharing a Pear Application](sharing-pear-application.md).
- Pair configuration values with runtime features in [Pear Runtime Essentials](pear-runtime.md).
- Validate CLI usage through [Pear CLI Reference](pear-cli.md).
