...
summary: Build a terminal-based Pear chat app using Bare runtime patterns and Hyperswarm networking.
tags: [pear, terminal, tutorial]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Flesh out a Pear terminal chat app with networking and UI flows.
> - **Primary APIs:** Pear runtime modules, Hyperswarm connections, Autobase/Hyperbee integrations.
> - **Protocols/Feeds:** Command topics over Protomux, Autobase feeds for chat transcripts.
> - **Dependencies:** Pear CLI, Bare runtime, Hyperswarm/Hyper modules, terminal UI helpers.
> - **Outputs:** Terminal chat client, replication pipeline, command handlers.
...
It continues where [Starting a Pear Terminal Project](starting-pear-terminal-app.md) left off.

{% embed url="<https://www.youtube.com/watch?v=UoGJ7PtAwtI>" %}
...
For the chat part of the app, the same modules are needed as in [Making a Pear Desktop Application](making-pear-application.md), `hyperswarm`, `b4a` and `hypercore-crypto`.
...
- Review runtime configuration in [Pear Application Configuration](app-config-and-package-json.md).
- Capture CLI troubleshooting workflows in [Debugging Pear Applications](debugging-pear-app.md).
