---
title: Dagify Effect Namespace
summary: Reference for commands, bridges, sinks, triggers, dispatcher, and invokeOnNode helpers.
tags: [dagify, effect, reference]
updated: 2025-11-21
audience: both
---
...
Helpers for boundaries where Dagify meets the outside world. Use these at application edges or explicit sink nodes to stay within the Agent Directive (no `.subscribe()` in library code).
...
- `invokeOnNode(node, method, ...args)`: call a method on a node (or observable) and emit results; helpful for teardown helpers.
