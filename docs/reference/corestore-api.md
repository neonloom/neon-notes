---
title: Corestore API Reference
summary: Explains Corestore key derivation, namespacing, replication, and session mechanics for managing Hypercores.
tags: [corestore, reference, api]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Corestore key derivation, session management, and replication orchestration.
> - **Primary APIs:** `corestore(namespace)`, `store.get()`, `store.namespace()`, `store.replicate()`.
> - **Protocols/Feeds:** Hypercore discovery keys, replication streams, namespace convention (`name:key`).
> - **Dependencies:** Node.js ≥18, `corestore`, `hypercore`, optional `hyperdrive`, `autobase`.
> - **Outputs:** Namespaced Hypercore sessions, replication streams, resource cleanup.
> - **Next Hop:** [`../walkthroughs/corestore-setup-walkthrough.md`](../walkthroughs/corestore-setup-walkthrough.md)

> Walkthrough companion: `../walkthroughs/corestore-setup-walkthrough.md`
...
## Next Steps
- Configure namespaces step-by-step with the [Corestore Setup Walkthrough](../walkthroughs/corestore-setup-walkthrough.md).
- Review replication interplay alongside [Hyperswarm API](hyperswarm-api.md).
- Pair Corestore usage with [Autobase API Reference](autobase-api.md) when assembling multiwriter resources.
