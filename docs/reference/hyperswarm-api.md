---
title: hyperswarm API Reference
summary: High-level peer discovery and connectivity API for topic-based networking over HyperDHT.
tags: [hyperswarm, networking, reference]
updated: 2025-10-18
audience: both
---
...
> **Context Card**
> - **Scope:** Hyperswarm peer discovery, topic joins, and connection management.
> - **Primary APIs:** `new Hyperswarm()`, `swarm.join()`, `swarm.replicate()`, `swarm.flush()`.
> - **Protocols/Feeds:** HyperDHT (Noise streams), 32-byte topic buffers, Hyperswarm discovery.
> - **Dependencies:** Node.js ≥18, `hyperswarm`, optional `hyperdht`, `noise-handshake`.
> - **Outputs:** Duplex connections, peer maps, replication streams.
> - **Next Hop:** [`../walkthroughs/hyperswarm-discovery-walkthrough.md`](../walkthroughs/hyperswarm-discovery-walkthrough.md)

> Walkthrough companion: `../walkthroughs/hyperswarm-discovery-walkthrough.md`
...
For CI and local development, we recommend using a Hyperswarm mock to avoid socket and NAT issues:

- Use a `FakeSwarm` (injectable shim) that simulates `join()` and emits `connection` events with in‑memory framed Duplex pairs.
- Inject via `startSwarmServer({ ..., swarmFactory })` and `startSwarmClientPool({ ..., swarmFactory })`.
- This keeps tests deterministic and fast without depending on network conditions.

For live verification (opt‑in):

```
RUN_SWARM_SMOKE=1 node scripts/swarm-smoke.js
```

This runs a minimal announce/lookup smoke using real Hyperswarm. Use only on hosts where UDP/TCP are available.

### Operational Guidance
- `maxPeers`: Keep a small cap (e.g., 4) to limit churn and resource use.
- `locality`: Prefer LAN transports (e.g., WebSocket bridges) when configured; fall back to Swarm for everything else. If your runtime maintains a peer pool, weight transports so local links are favoured before relaying.
- `capabilities`: If exposing management endpoints over Swarm, enforce capability tokens on mutations.
...
## Next Steps
- Study connection lifecycles in the [Hyperswarm Discovery Walkthrough](../walkthroughs/hyperswarm-discovery-walkthrough.md).
- Combine networking with [HyperDHT API Reference](hyperdht-api.md) when tuning bootstrap servers.
- Coordinate higher-level protocols using the [Protomux API Reference](protomux-api.md).
